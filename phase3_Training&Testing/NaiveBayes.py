...
data_train_smote, target_train_smote = SMOTE(random_state = seed).fit_sample(data_train, target_train)
...
for train, test in cv.split(N_data, target):
    data_train_cv_smote, target_train_cv_smote = SMOTE(random_state = seed).fit_sample(N_data[train], target[train])
